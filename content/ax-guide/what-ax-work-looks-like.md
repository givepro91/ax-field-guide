...
slides:
  ...
  - kick: 테제
    title: 모델이 아니라 다섯 자리
    body: 맡은 사람의 일은 더 똑똑한 모델을 고르는 게 아니라, 다섯 자리에서 판단을 내리는 것.
  - kick: 지도
  ...
  - kick: 1. 문제
    title: 시연용 ≠ 가치 있는
    body: 가장 시연하기 좋은 루프가 가장 값있는 경우는 드물다. 빈도·경계·회복 가능성으로 거른다.
  - kick: 4. 게이트
    title: 위험이 클수록 앞에 둔다
    body: 자동 적용할지, 제안 후 사람을 기다릴지. 게이트의 위치가 신뢰를 만든다.
  - kick: 5. 폐기
    title: 접는 판단이 더 어렵다
    body: 만든 자동화를 언제 끄나. 드리프트·비용·신뢰 하락이 신호다. 안 접으면 빚이 된다.
    cover: true
---
