...
slides:
  - kick: AX Field Guide · 개념·관점
    title: AI 출력을 어디까지 믿을 것인가
    body: 신뢰는 정확도 하나로 정해지지 않는다
    cover: true
  - kick: 테제
    title: 신뢰 = 정확도만이 아니다
    body: 같은 정확도라도, 틀렸을 때 회복이 쉬우면 더 믿고 어려우면 덜 믿는다.
  - kick: 세 변수
    title: 정확도 · 회복 비용 · 검증 비용
    bullets:
      - 얼마나 자주 맞나
      - 틀리면 되돌리는 값
      - 맞는지 확인하는 값
  - kick: 관계
    title: 검증이 회복보다 싸면 검증한다
    body: 확인 비용이 사고 비용보다 작을 때만 사람이 검증하는 게 값을 한다.
  - kick: 전략
    title: 다 볼 필요는 없다
    body: 전수·샘플·자동·무검증. 위험에 맞춰 검증의 강도를 고른다.
  - kick: 정리
    title: 신뢰는 고정값이 아니다
    body: 입력이 바뀌면 정확도도 변한다. 한 번 믿었다고 계속 믿지 않는다.
    cover: true
---
