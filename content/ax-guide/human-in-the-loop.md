...
slides:
  - kick: AX Field Guide · 패턴·플레이북
    ...
    body: 사람이 "아니오"를 말할 수 있는 자리를 설계하는 일
    cover: true
  - kick: 테제
    title: 자리보다 권한이다
    body: 사람을 끼우는 것만으로는 부족하다. 그 자리에서 진짜 거부할 수 있어야 한다.
  - kick: 세 자리
    title: 사전 · 사후 · 예외
    bullets:
      - 사전 승인 — 적용 전에 막는다
      - 사후 검토 — 적용 후 되돌린다
      - 예외 에스컬레이션 — 이상할 때만 부른다
  - kick: 실패
    title: 형식이 된 개입
    body: 맥락 없이, 거부가 어렵고, 양이 많으면 — 사람은 그냥 통과시킨다.
  - kick: 설계
    title: 거부를 쉽게, 양을 줄여
    body: 판단할 맥락을 주고, 아니오를 예만큼 쉽게, 저위험은 자동으로 빼 양을 줄인다.
  - kick: 정리
    title: 사람은 비싼 자원이다
    body: 모든 곳에 사람을 두면 자동화가 사라진다. 사람의 판단은 가장 위험한 자리에 아껴 쓴다.
    cover: true
---
